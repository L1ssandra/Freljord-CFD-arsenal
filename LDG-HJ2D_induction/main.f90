    ...
    parameter(Nx0 = 320, Ny0 = 320, Lphi = 0, kk = 3, NumEq = 2, NumGLP = 5, RKorder = 4, flux_type = 1)
    ...
    real(8) p1ij(NumEq),p2ij(NumEq),q1ij(NumEq),q2ij(NumEq),Hhat1(NumEq),Hij(NumEq)
    real(8) SRx,SLx,SRy,SLy
    ...
    real(8) Hhat(NumGLP,NumGLP,0:Nphi,NumEq)
    ...
    real(8) p1G(NumGLP,NumGLP,0:Nphi,NumEq),p2G(NumGLP,NumGLP,0:Nphi,NumEq)
    real(8) q1G(NumGLP,NumGLP,0:Nphi,NumEq),q2G(NumGLP,NumGLP,0:Nphi,NumEq)
            ...
            do k = 0,Nphi
                do j1 = 1,NumGLP
                    do i1 = 1,NumGLP
                        ...
                        Hij(1) = 0.5*(q1ij(1) + q2ij(1)) - 0.5*(q1ij(2) + q2ij(2))
                        Hij(2) = 0.5*(p1ij(2) + p2ij(2)) - 0.5*(p1ij(1) + p2ij(1))
    ...
    Hhat1 = Hij - 0.5*max(abs(SRx),abs(SLx))*(p1ij - p2ij) - 0.5*max(abs(SRy),abs(SLy))*(q1ij - q2ij)
    
    end subroutine HhatLF
