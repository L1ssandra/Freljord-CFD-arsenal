...
for i = 1:Nx
    for n = 1:NumEq
        deltaUR(n,1) = uh(i,2,n) + (2/3)*uh(i,3,n) + (2/5)*uh(i,4,n) + (8/35)*uh(i,5,n);
        deltaUL(n,1) = uh(i,2,n) - (2/3)*uh(i,3,n) + (2/5)*uh(i,4,n) - (8/35)*uh(i,5,n);
    ...
    for n = 1:NumEq
        uhmod(i,2,n) = 7/10*uh(i,2,n) - 3/25*uh(i,4,n) + 3*(deltaURM1(n) + deltaULM1(n))/20;
        uhmod(i,3,n) = 9/14*uh(i,3,n) - 6/49*uh(i,5,n) + 3*(deltaURM1(n) - deltaULM1(n))/4;
        uhmod(i,4,n) = -7/4*uh(i,2,n) + 3/10*uh(i,4,n) + 7*(deltaURM1(n) + deltaULM1(n))/8;
        uhmod(i,5,n) = -15/8*uh(i,3,n) + 5/14*uh(i,5,n) + 45*(deltaURM1(n) - deltaULM1(n))/32;
    end
end
