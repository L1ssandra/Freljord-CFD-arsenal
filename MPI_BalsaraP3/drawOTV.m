...
figure(1);
contour(rc,zc,Qrho,30);colormap(cool);
%p = pcolor(rc,zc,Qrho);colormap(bone);p.EdgeColor = 'none';colorbar;
%mesh(xc,yc,Q1);
% title('Density')
...
QBr = 0.5*(Q6(Nr/2,:) + Q6(Nr/2 + 1,:));
plot(Zc,QBr,'k-','linewidth',1.3);
% plot(Yc,Bx(Nx/10,:),'r-','linewidth',1.3)
% title('Br cut at r = 1.5')
axis([Zc(1),Zc(end),min(QBr) - 0.1,max(QBr) + 0.1])